...
class MultiAgentEnv(gym.Env):
    ...
    def step(self, action_n):
        ...
        obstacles_info = []
        for i in range(self.world.num_obstacles):
            obstacles_info.append([self.world.obstacles[i + self.world.num_obstacles].state.p_pos[0],
                                   self.world.obstacles[i + self.world.num_obstacles].state.p_pos[1],
                                   self.world.obstacles[i + self.world.num_obstacles].size])
        landmarks_info = []
        for i in range(self.world.num_landmarks):
            landmarks_info.append([self.world.landmarks[i + self.world.num_landmarks].state.p_pos[0],
                                   self.world.landmarks[i + self.world.num_landmarks].state.p_pos[1],
                                   self.world.landmarks[i + self.world.num_landmarks].size])
        for i in range(self.world.num_agents):
            if action_n[i][0] == 100:
                action_n[i][0] = A_Star(self.world.num_agents, self.world.num_obstacles, self.world.num_landmarks,
                                     agents_info, obstacles_info, landmarks_info, self.world.angle,i,i)
