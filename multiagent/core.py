        ...
        for i,entity in enumerate(self.entities):
            ...
            if 'agent' in entity.name:
                ...
                if (p_force[i] is not None):
                    ...
                    #原代码中的转向采用的是平滑转向，并不是直接转为推力方向，采取*0，3的方式来延缓方向的转动，以此模拟真实船只掉头
                    #self.angle[i] += p_force[i].item(0)*0.3 #multiply by 0.1 to set radius limit at 100m minimum
                    #此处修改暂时抛弃这一现实因素，后续在变速运动时再考虑将力与当前速度方向状态进行叠加，此时的匀速运动暂不考虑转向延迟
                    self.angle[i] += p_force[i].item(0)
                ...
                vel = 0.001 #seting this velocity (0.1) and considering that the dt is equal to 0.1, means that we have a new position each 10m. #给定了匀速
                if vel < 0:
                    vel = 0
                #Finally, we increase the position (aka the agent) using the new angle and velocity.
                entity.state.p_pos += np.array([vel*np.cos(self.angle[i]),vel*np.sin(self.angle[i])]) * self.dt #实体位置发生移动
